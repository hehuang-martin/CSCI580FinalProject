...
def local(hdr, saturation=1., gamma=1/2.2):
    params = [cv2.IMWRITE_JPEG_QUALITY, 80] # jpeg quality params
  
    # compute tonemapped image
    local_tonemap = tonemap(hdr, saturation=saturation, gamma_=gamma, numtiles=(8,8))
  
    # plot and save locally tonemapped image
    cv2.imwrite("local.jpg", local_tonemap[:,:,[2,1,0]], params)

def reinhard_tonemap(irradiance_map, gamma=1/6, alpha=0.25):
    C = 3 # num of channels
    ...
    ldr = np.clip(tonemapped_img, 0.0, 1.0)
    cv2.imwrite('{}.png'.format('reinhard_manual.png'), ldr * 255)
